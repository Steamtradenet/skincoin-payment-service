            ...
            if (!StringUtils.isEmpty(app.getCallbackUrl()) && !hasHold(app.getId())) {
                try {
                    ...
                    for (Notification notification: notifications) {
                        ...
                        SimpleSuccessJson response = restTemplate.postForObject(app.getCallbackUrl(), data, SimpleSuccessJson.class);
                        if (response != null &&  response.getSuccess() != null && response.getSuccess()) {
                            notificationDao.delete(notification); // Remove notification
                        }
                    }
                } catch (Exception ex) {
                    delayMap.put(app.getId(), DateUtils.addMinutes(new Date(), 5));
                    log.error(this.getClass(), "Can't notify app(id:"+app.getId()+"). Try notify in 5 min", ex);
                }
            }
        }
    }
